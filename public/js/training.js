...
const training = getTrainingById(selectedType);
renderInputByFormat(training.format);
trainingInput.format = training.format
...
function renderStyleTabs() {
  const tabsHTML = TRAINING_STYLES.map(style => `
    <button class="style-tab ${style.id === selectedStyle ? 'active' : ''}" data-style="${style.id}">
      ${style.name}
    </button>
  `).join('');
...
function renderTrainingTypes() {
  // カテゴリとスタイル両方で絞り込み
  const types = getTrainingsByCategoryAndStyle(selectedCategory, selectedStyle);
